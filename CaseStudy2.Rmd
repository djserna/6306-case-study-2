...
```{r analysis}
#doesn't work - states x must be nurmeric
#correlationMatrix <- cor(as.matrix(MHData[,21:31]), use="complete.obs")

# library(tidyr)
# test <- separate(MHSubset, MHSubset$MHCurrentlyDiagnosed, sep="\\|")
# 
# test2 <- separate(MHSubset[2,"MHCurrentlyDiagnosed"], "|")

# MHDiagnoses <- sqldf("
#  SELECT A.[CurrentPosition],  
#      Split.a.value('.', 'VARCHAR(500)') AS String  
#  FROM  (SELECT [CurrentPosition],  
#          CAST ('<M>' + REPLACE([MHCurrentlyDiagnosed], ',', '</M><M>') + '</M>' AS XML) AS String  
#      FROM MHSubset) AS A CROSS APPLY String.nodes ('/M') AS Split(a);
# ")

splitDiagnosis <- function(row){
  diagnoses <- unlist(strsplit(as.character(row$MHCurrentlyDiagnosed), "\\|"))
  positions <- unlist(strsplit(as.character(row$CurrentPosition), "\\|"))
  returnValue <- data.frame(CurrentPosition = positions, Diagnoses = diagnoses)
  returnValue
}

# newDF <- data.frame()
# for (row in 1:nrow(MHSubset)){
#   newRow <- splitDiagnosis(MHSubset[row,])
#   rbind(newDF, newRow)
# }
