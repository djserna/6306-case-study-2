...
*District of Columbia was removed as an outlier in the data set*
...
fit1 <- glm(NUM_DiscussMHWithBoss ~ HasSubstanceAbuse, data=MHSubstanceAbuseSupervisor, family = binomial )
summary(fit1)
```
...
fit1 <- glm(NUM_AnonymityProtected ~ HasSubstanceAbuse, data=MHSubstanceAbuseAnonymity, family = binomial )
summary(fit1)
```
...
The team subsetted the data to include the columns of 'HasSubstanceAbuse' and 'NUM_NegResponseWithMH'.  The NUM_NegResponseWithMH column is used to indicate if the observation reports observing or experiencing a negative repsonse with mental health issues in the workplace. The HasSubstanceAbuse column indicates if the observation's mental health issue relates to substance abuse.  The team provided multiple ways to cross check the results to ensure that the results were verified.
...
A proportion table is provided that shows the percentages of workers with mental health issues that report observing or experiencing a negative response to a mental health issue. It appears that workers with a substance abuse disorder are more likely to observe or experience a negativ reponse when compared to other mental health issues. 71.43% of workers with substance abuse disorder report a negative response compared to 54.99% of workers with other mental health issues. It is disturbing to see such a high report of negative responses for workers with substance abuse disorders.
...
fit1 <- glm(NUM_NegResponseWithMH ~ HasSubstanceAbuse, data=MHSubstanceAbuseNegResponse, family = binomial )
summary(fit1)
```
